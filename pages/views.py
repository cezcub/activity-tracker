...
def awards_view(request):
	context = {
		"adult_miles": Participant.objects.filter(age_group="18+").annotate(total_miles=Cast(Sum('activity__miles'), IntegerField())).order_by("-total_miles")[:1],
		...
		"adult_longest_bike": Participant.objects.filter(activity__activity_type='Biking', age_group="18+").annotate(most_miles=Max("activity__miles")*2).order_by("-most_miles")[:1],
		...
		"child_miles": Participant.objects.exclude(age_group="18+").annotate(total_miles=Cast(Sum('activity__miles'), IntegerField())).order_by("-total_miles")[:1],
